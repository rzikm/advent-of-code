...
let parse input =
    ...
    let pstring (str : string) = str.Split([|'\n'; '\r'|], System.StringSplitOptions.RemoveEmptyEntries) |> Seq.map (fun s -> pstring s) |> Seq.reduce (fun l r -> l .>> pNewline >>. r)
    let pSequence = 
        pstring "inp w
mul x 0
add x z
mod x 26
div z " >>. ((pstring "1" >>. preturn true) <|> (pstring "26" >>. preturn false)) .>> pNewline .>> pstring "
add x " .>>. pint32 .>> pNewline .>> pstring "
eql x w
eql x 0
mul y 0
add y 25
...
mul y 0
add y w
add y " .>>. pint32 .>> pNewline .>> pstring "
mul y x
add z y" |>> (fun ((upDown, v1), v2) -> (upDown, v1, v2))
    let pProgram = parray 14 (pSequence .>> pNewline) |>> List.ofArray
...
let solve minmax prg =
    ...
    f prg [] (Array.zeroCreate 14) 0
    |> Array.map int64 |> Array.reduce (fun l r -> l * 10L + r)

let part1 input = solve max input
let part2 input = solve min input
...
module Unused = 
    ...
    let parse input =
        let pNewline = pchar '\n'
        let pReg = [ ('w', W); ('x', X); ('y', Y); ('z', Z) ] |> List.map (fun (c, r) ->
            pchar c >>. preturn r) |> choice
        let pRegOrNum = (pReg |>> Register) <|> (pint32 |>> Number)
                
        let pInp = pstring "inp" >>. spaces >>. pReg |>> (fun r -> Instruction (r, Inp))
        let pBinOp = [("add", Add); ("mul", Mul); ("div", Div); ("mod", Mod); ("eql", Eql)] |> List.map (fun (str, factory) ->
            pstring str >>. spaces >>. pReg .>> spaces .>>. pRegOrNum |>> (fun (t, r) -> Instruction (t, factory r))) |> choice
