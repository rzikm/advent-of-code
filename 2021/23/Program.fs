...
type State = {
    hallway : Amphipod option []
    rooms : Amphipod list []
}

let parse input =
    let pAmphipodType c t = pchar c >>. preturn t
    let pAmphipod = choice [ pAmphipodType 'A' Amber; pAmphipodType 'B' Bronze; pAmphipodType 'C' Copper; pAmphipodType 'D' Desert ]
    let pNewline = pchar '\n'
    let pTop = pstring "#############"
    ...
    let initState (top, bottom) =
        let pick i = [ (Array.item i top); (Array.item i bottom) ]
        {
...
let roomIndexToAmphipod = function
    | 0 -> Amber
    | 1 -> Bronze
    | 2 -> Copper
    | 3 -> Desert

let getTargetRoomIndex = function
    | Amber -> 0
    | Bronze -> 1
    | Copper -> 2
    | Desert -> 3

let getStepCost = function
    | Amber -> 1
    | Bronze -> 10
    | Copper -> 100
    | Desert -> 1000

let hallwaySpaceHorizontalOffset = function
    // #############
    // #01.2.3.4.56#
    // ###B#C#B#D###
    //   #A#D#C#A#
...
let roomHorizontalOffset = function
    // #############
    // #..0.1.2.3..#
    // ###B#C#B#D###
    //   #A#D#C#A#
        ...
        for roomIdx in 0..3 do
            ...
            | amph::otherAmphs when room |> List.exists (getTargetRoomIndex >> (<>) roomIdx) ->
                ...
                for hallwaySpaceIndex in candidates do
                    let horizontalDist = abs (roomHorizontalOffset roomIdx - hallwaySpaceHorizontalOffset hallwaySpaceIndex)
...
let printState roomSize (state : State) =
    ...
    //   #########"
    printfn "#############"
    printfn "#%s#" (state.hallway |> Seq.map (Option.map amphToString >> Option.defaultValue ".")
        |> Seq.insertAt 2 "." |> Seq.insertAt 4 "." |> Seq.insertAt 6 "." |> Seq.insertAt 8 "." |> String.concat "")
...
let fHeuristic roomSize (state : State) =
    state.rooms
    |> Seq.indexed |> Seq.sumBy (fun (i, room) ->
            let size = List.length room

            // cost of filling the room with correct amphipods
            let countToGo = roomSize - (room |> Seq.filter (getTargetRoomIndex >> (=) i) |> Seq.length)
            let fillingCost = countToGo * (countToGo - 1) * (roomIndexToAmphipod i |> getStepCost)

            // cost of removing unnecessary amphipods
            let costOfRemoval = 
                room |> Seq.indexed |> Seq.filter (snd >> getTargetRoomIndex >> (<>) i) 
...
###B#C#B#D###
  ...
  #########" |> parse
    ...
    let ``Generates steps from room to the left`` () =
        ...
        getNeighbors 2 state |> List.ofSeq |> should equal [
            ({
    ...
    [<Fact>]
    let ``Example Part 1`` () =
        exampleInput () |> part1 |> should equal 12521

    [<Fact>]
    let ``Example Part 2`` () =
        exampleInput () |> part2 |> should equal 44169
