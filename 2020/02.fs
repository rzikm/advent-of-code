...
let parser =
    let line =
        tuple3 (pint32 .>> pchar '-' .>>. pint32) (spaces >>. anyChar .>> pchar ':') (spaces >>. many1CharsTill anyChar (pchar '\n'))
    many line

let isMatchEasy (min, max) c pass =
    let count = pass |> Seq.countBy id |> Seq.tryFind (fst >> (=) c) |> Option.map snd |> Option.defaultValue 0
    min <= count && count <= max
