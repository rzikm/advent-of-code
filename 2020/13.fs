...
let waitTime departure id =
    (departure + id - 1L) / id * id - departure

let easy (departure, busses) =
    busses |> List.choose id |> List.map (fun id -> (waitTime departure id), id) |> List.minBy fst ||> (*)

let hard (_, busses) =
    let busses =
        busses |> List.choosei (fun i b -> (b |> Option.map (fun b -> (int64 i, b))))
