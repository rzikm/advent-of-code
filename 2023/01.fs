...
let parser = sepEndBy1 (manyChars <| satisfy (fun c -> isLetter c || isDigit c)) (pchar '\n')

let digits = [ 1..9 ] |> List.map (fun d -> string d, d)
let digitsAsWords = [ "one", 1; "two", 2; "three", 3; "four", 4; "five", 5; "six", 6; "seven", 7; "eight", 8; "nine", 9 ]
...
let solution = makeSolution parser (solve digits) (solve <| List.append digits digitsAsWords)
